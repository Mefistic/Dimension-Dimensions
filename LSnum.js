...
class LSN {
  ...
  floor() {
    var ret = new LSN(this)
    if (ret.l < 15)
      ret.l = Math.log10(Math.floor(Math.pow(10, ret.l + LSN_EPSILON)))
    return ret
  }
  ...
  ceil() {
    var ret = new LSN(this)
    if (ret.l < 15)
      ret.l = Math.log10(Math.ceil(Math.pow(10, ret.l + LSN_EPSILON)))
    return ret
  }
  ...
  trunc() {
    var ret = new LSN(this)
    if (ret.l < 15)
      ret.l = Math.log10(Math.trunc(Math.pow(10, ret.l + LSN_EPSILON)))
    return ret
  }
  ...
  round() {
    var ret = new LSN(this)
    if (ret.l < 15)
      ret.l = Math.log10(Math.round(Math.pow(10, ret.l + LSN_EPSILON)))
    return ret
  }
  ...
  lt(x) {
    x = new LSN(x)
    return this.l * 1e-317 < x.l * 1e-317
  }
  ...
  static sci(x, places = 2, placSci = 2) {
    ...
    if (x.toFixed(places) >= 1e1)
      places = Math.max(places - 1, 0)
    if (x.toFixed(places) >= 1e2)
      places = Math.max(places - 1, 0)
    ...
    placSci = Math.max(placSci, 0)

    if (x.lt(`1e100000000000`)) {
      if (x.m.toFixed(placSci) == 10)
        return ((1).toFixed(placSci) + `e` + (x.e + 1).toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,"))
      else
        return (x.m.toFixed(placSci) + `e` + x.e.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,"))
    }
